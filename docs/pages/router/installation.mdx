...
For the property `main`, use the `expo-router/entry` as its value in the **package.json**. The initial client file is [**app/\_layout.jsx**](/router/advanced/root-layout) if you are developing with JavaScript, or [**app/\_layout.tsx**](/router/advanced/root-layout) if you are developing with TypeScript.
